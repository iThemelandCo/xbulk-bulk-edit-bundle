<div class="wpbel-top-nav">
    <div class="wpbel-top-nav-buttons" id="wpbel-bulk-edit-navigation">
        ...
        <div class="wpbel-top-nav-buttons-group">
            ...
            <button type="button" data-toggle="modal" data-post-type="<?php echo esc_attr($GLOBALS['wpbel_common']['active_post_type']); ?>" data-target="#wpbel-modal-new-item"><?php esc_html_e('New ' . str_replace('_', ' ', ucfirst(esc_html($GLOBALS['wpbel_common']['active_post_type']))), WBEBL_NAME); ?></button>
        </div>
        <div class="wpbel-top-nav-buttons-border"></div>
        <div class="wpbel-bulk-edit-form-selection-tools">
            <div class="wpbel-top-nav-buttons-group">
                <button type="button" id="wpbel-bulk-edit-unselect"><?php esc_html_e('Unselect', WBEBL_NAME); ?></button>
                <button type="button" id="wpbel-bulk-edit-duplicate" data-toggle="modal" data-target="#wpbel-modal-item-duplicate"><?php esc_html_e('Duplicate', WBEBL_NAME); ?>
                </button>
    ...
    <div class="wpbel-top-nav-filters-switcher">
        <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" id="wpbel-switcher-form">
            ...
            <?php wp_nonce_field(); ?>
            <label for="wpbel-switcher"><?php esc_html_e('Select post type', WBEBL_NAME); ?></label>
            <select id="wpbel-switcher" name="post_type" title="<?php esc_html_e('Select post type', WBEBL_NAME); ?>">
                <?php if (!empty($post_types)) : ?>
                    <?php foreach ($post_types as $post_type_key => $post_type_label) : ?>
                        <option value="<?php echo esc_attr($post_type_key) ?>" <?php echo ($GLOBALS['wpbel_common']['active_post_type'] == $post_type_key) ? esc_attr('selected') : ''; ?>>
                            <?php echo esc_attr($post_type_label); ?>
                        </option>
                    <?php endforeach; ?>
                <?php endif; ?>
            </select>
        ...
        <div class="wpbel-top-nav-filters-left">
            ...
            <?php if (isset($settings['show_quick_search']) && $settings['show_quick_search'] == 'yes') : ?>
                ...
                <div class="wpbel-top-nav-filters-search">
                    <input type="text" id="wpbel-quick-search-text" placeholder="<?php esc_html_e('Quick Search ...', WBEBL_NAME); ?>" title="Quick Search" value="<?php echo (isset($quick_search_input['quick_search_text'])) ? esc_attr($quick_search_input['quick_search_text']) : '' ?>">
