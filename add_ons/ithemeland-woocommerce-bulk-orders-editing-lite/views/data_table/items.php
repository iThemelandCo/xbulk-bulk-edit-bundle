<table id="wobef-items-list">
        ...
        <tr>
            <?php if (isset($show_id_column) && $show_id_column === true) : ?>
                ...
                <th class="wobef-td70 <?php echo ($sticky_first_columns == 'yes') ? 'wobef-td-sticky wobef-td-sticky-id' : ''; ?>">
                    <input type="checkbox" class="wobef-check-item-main" title="<?php esc_html_e('Select All', WBEBL_NAME); ?>">
                    <label data-column-name="id" class="wobef-sortable-column"><?php esc_html_e('ID', WBEBL_NAME); ?><span class="wobef-sortable-column-icon"><?php echo sprintf('%s', $sortable_icon); ?></span></span>
                </th>
            <?php endif; ?>
            <?php if (!empty($next_static_columns)) : ?>
                <?php foreach ($next_static_columns as $static_column) : ?>
                    ...
                    ?>
                    <th data-column-name="<?php echo esc_attr($static_column['field']) ?>" class="wobef-sortable-column wobef-td120 <?php echo ($sticky_first_columns == 'yes') ? 'wobef-td-sticky wobef-td-sticky-title' : ''; ?>"><?php echo esc_html($static_column['title']); ?><span class="wobef-sortable-column-icon"><?php echo sprintf('%s', $sortable_icon); ?></span></th>
                <?php endforeach; ?>
            <?php endif; ?>
            ...
            <?php if (!empty($after_dynamic_columns)) : ?>
                <?php foreach ($after_dynamic_columns as $last_column_item) : ?>
                    <th data-column-name="<?php echo esc_attr($last_column_item['field']) ?>" class="wobef-td120"><?php echo esc_html($last_column_item['title']); ?></th>
                <?php endforeach; ?>
            <?php endif; ?>
        </tr>
    </thead>
    <tbody>
        <?php if (!empty($items_loading)) : ?>
            <tr>
                <td colspan="8" class="wobef-text-alert"><?php esc_html_e('Loading ...', WBEBL_NAME); ?></td>
            </tr>
        ...
        <?php else : ?>
            <tr>
                <td colspan="8" class="wobef-text-alert"><?php esc_html_e('No Data Available!', WBEBL_NAME); ?></td>
            </tr>
        <?php endif; ?>
    </tbody>
    <tfoot>
