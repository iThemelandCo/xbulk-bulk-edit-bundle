<table id="wccbef-items-list">
        ...
        <tr>
            <?php if (isset($show_id_column) && $show_id_column === true) : ?>
                ...
                <th class="wccbef-td70 <?php echo ($sticky_first_columns == 'yes') ? 'wccbef-td-sticky wccbef-td-sticky-id' : ''; ?>">
                    <input type="checkbox" class="wccbef-check-item-main" title="<?php esc_html_e('Select All', WBEBL_NAME); ?>">
                    <label data-column-name="id" class="wccbef-sortable-column"><?php esc_html_e('ID', WBEBL_NAME); ?><span class="wccbef-sortable-column-icon"><?php echo sprintf('%s', $sortable_icon); ?></span></span>
                </th>
            <?php endif; ?>
            <?php if (!empty($next_static_columns)) : ?>
                <?php foreach ($next_static_columns as $static_column) : ?>
                    ...
                    ?>
                    <th data-column-name="<?php echo esc_attr($static_column['field']) ?>" class="wccbef-sortable-column wccbef-td120 <?php echo ($sticky_first_columns == 'yes') ? 'wccbef-td-sticky wccbef-td-sticky-title' : ''; ?>"><?php echo esc_html($static_column['title']); ?><span class="wccbef-sortable-column-icon"><?php echo sprintf('%s', $sortable_icon); ?></span></th>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <tbody>
        <?php if (!empty($items_loading)) : ?>
            <tr>
                <td colspan="8" class="wccbef-text-alert"><?php esc_html_e('Loading ...', WBEBL_NAME); ?></td>
            </tr>
        ...
        <?php else : ?>
            <tr>
                <td colspan="8" class="wccbef-text-alert"><?php esc_html_e('No Data Available!', WBEBL_NAME); ?></td>
            </tr>
        <?php endif; ?>
    </tbody>
    <tfoot>
